    ...
    async function createSubtreePropsFromSegmentPath(segments) {
        ...
        for (const [parallelKey, parallelSegment] of parallelSegments){
          ...
          ${(0, _discover.createMetadataExportsCode)(metadata)}
            ...
            if (!rootLayout) {
                ...
                if (layoutPath) {
                    const resolvedGlobalErrorPath = await resolver(`${_path.default.dirname(layoutPath)}/${GLOBAL_ERROR_FILE_TYPE}`);
        ...
        for (const adjacentParallelSegment of adjacentParallelSegments){
            if (!props[normalizeParallelKey(adjacentParallelSegment)]) {
                ...
                let defaultPath = await resolver(`${appDirPrefix}${segmentPath}/${actualSegment}/default`);
                if (!defaultPath) {
                    ...
                    const normalizedDefault = await resolver(`${appDirPrefix}${(0, _apppaths.normalizeAppPath)(segmentPath)}/${actualSegment}/default`);
                    // if a default is found, use that. Otherwise use the fallback, which will trigger a `notFound()`
                    defaultPath = normalizedDefault ?? _parallelroutedefault.PARALLEL_ROUTE_DEFAULT_PATH;
                }
